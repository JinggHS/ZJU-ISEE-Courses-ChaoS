%  EM4 Homework Q2(b)
%  Wavelength-angle diagrams for a 10-layer DBR with a defect layer
%  Center wavelength = 500 nm
%  TM and TE are plotted together for each refractive-index-ratio case
%  The 5th layer is changed from n1 to (n1+n2)/2
%  Thicknesses of all layers remain unchanged
...
plot_case_defect(y, theta, n1, n2, lambda0, 'Defect case: n1/n2 = 2  (n1 = 3, n2 = 1.5)');
...
n1 = 6.0;
lambda0 = 500;   % center wavelength (nm)
plot_case_defect(y, theta, n1, n2, lambda0, 'Defect case: n1/n2 = 4  (n1 = 6, n2 = 1.5)');

end


function plot_case_defect(y, theta, n1, n2, lambda0, fig_title_text)
...
for m = 1:length(theta)
    ...
    % TM
    res_TM(m, :) = G_Ref_defect(y, theta(m), 1, n1, n2, lambda0);

    % TE
    res_TE(m, :) = G_Ref_defect(y, theta(m), 0, n1, n2, lambda0);
end
...
function res = G_Ref_defect(y, theta, flag, n1, n2, lambda0)
...
% quarter-wave thickness design of the original perfect DBR
d1 = (lambda0/4) / n1;    % nm
d2 = (lambda0/4) / n2;    % nm

% keep all thicknesses unchanged
...
% defect layer refractive index: (n1+n2)/2
nd = 0.5 * (n1 + n2);
Def = (nd)^2 * ones(length(y),1);

res = zeros(1, length(y));

for fre = 1:length(x)

    % Original perfect stack:
    % air | n1 | n2 | n1 | n2 | n1 | n2 | n1 | n2 | n1 | n2 | air
    %
    % Defect stack for Q2(b):
    % change the 5th layer from n1 to nd
    %
    % air | n1 | n2 | n1 | n2 | nd | n2 | n1 | n2 | n1 | n2 | air

    epr_arr = [AIR(fre), Die1(fre), Die2(fre), Die1(fre), Die2(fre), ...
               Def(fre), Die2(fre), Die1(fre), Die2(fre), Die1(fre), ...
               Die2(fre), AIR(fre)];
